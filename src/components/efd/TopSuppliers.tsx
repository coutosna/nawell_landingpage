...
export const TopSuppliers: React.FC<TopSuppliersProps> = ({ efdTxt }) => {
  ...
  const suppliersData = useMemo(() => {
    ...
    const supplierAgg = new Map<string, { valorTotal: number; countNf: number; uf: string; nome: string; cnpj?: string }>();

    documentosC100.forEach((doc, docKey) => {
      // Filtrar apenas entradas (IND_OPER = 0)
      if (doc.indOper !== '0') return;
      ...
      if (!supplierAgg.has(doc.codPart)) {
        supplierAgg.set(doc.codPart, {
          valorTotal: 0,
          countNf: 0,
      ...
      const agg = supplierAgg.get(doc.codPart)!;
      agg.valorTotal += valorDoc;
      agg.countNf += 1;
    });

    // Converter para array e calcular métricas
    const suppliers: SupplierData[] = Array.from(supplierAgg.entries()).map(([codPart, data]) => ({
      codPart,
    ...
    suppliers.sort((a, b) => b.valorTotal - a.valorTotal);

    // Calcular participação e Pareto
    const totalGeral = suppliers.reduce((sum, s) => sum + s.valorTotal, 0);
    let acumulado = 0;

    suppliers.forEach(supplier => {
      supplier.participacao = totalGeral > 0 ? supplier.valorTotal / totalGeral : 0;
      acumulado += supplier.participacao;
      supplier.acumuladoPareto = acumulado;
    });

    return {
      suppliers,
      totalGeral,
      totalNfs: suppliers.reduce((sum, s) => sum + s.countNf, 0),
      totalSuppliers: suppliers.length,
    };
  }, [efdTxt]);

  const topSuppliers = suppliersData.suppliers.slice(0, topN);
  const top5Total = suppliersData.suppliers.slice(0, 5).reduce((sum, s) => sum + s.valorTotal, 0);
  const top5Percent = suppliersData.totalGeral > 0 ? top5Total / suppliersData.totalGeral : 0;
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="hover-lift group relative overflow-hidden">
          ...
          <CardContent className="overflow-hidden relative z-10">
            <div className="text-2xl lg:text-3xl font-bold break-words bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text">
              {formatCurrency(suppliersData.totalGeral)}
            </div>
            <p className="text-xs text-muted-foreground mt-2 font-medium">Em compras realizadas</p>
          </CardContent>
        </Card>
        ...
        <Card className="hover-lift group relative overflow-hidden">
          ...
          <CardContent className="relative z-10">
            <div className="text-3xl font-bold">{suppliersData.totalNfs.toLocaleString('pt-BR')}</div>
            <p className="text-xs text-muted-foreground mt-2 font-medium">Documentos recebidos</p>
          </CardContent>
        </Card>
        ...
        <Card className="hover-lift group relative overflow-hidden">
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2 relative z-10">
            <CardTitle className="text-sm font-semibold text-muted-foreground uppercase tracking-wider">Fornecedores</CardTitle>
          ...
          <CardContent className="relative z-10">
            <div className="text-3xl font-bold">{suppliersData.totalSuppliers}</div>
            <p className="text-xs text-muted-foreground mt-2 font-medium">Fornecedores ativos</p>
          </CardContent>
        </Card>

        <Card className="hover-lift group relative overflow-hidden border-2 border-accent/20">
          <div className="absolute inset-0 bg-gradient-to-br from-accent/10 to-primary/10" />
          <CardHeader className="flex flex-row items-center justify-between pb-2 relative z-10">
            <CardTitle className="text-sm font-semibold text-accent uppercase tracking-wider">Top 5 Concentração</CardTitle>
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-accent to-primary flex items-center justify-center shadow-lg shadow-accent/30 group-hover:scale-110 transition-transform duration-300">
              <TrendingUp className="h-5 w-5 text-white" />
            </div>
          </CardHeader>
          <CardContent className="relative z-10">
            <div className="text-3xl font-bold text-accent">{formatPercent(top5Percent)}</div>
            <p className="text-xs text-muted-foreground mt-2 font-medium">Dos 5 maiores fornecedores</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="hover-float border-2">
        <CardHeader className="space-y-4">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <CardTitle className="text-2xl">Top {topN} Fornecedores por Valor</CardTitle>
              <CardDescription className="text-base mt-2">
                Ranking dos principais fornecedores por valor total de entradas
              </CardDescription>
            </div>
          </div>
            ...
            {[10, 20, 50].map(n => (
              <Badge
                ...
                className={cn(
                  ...
                  topN === n 
                    ? "bg-gradient-to-r from-accent to-primary shadow-lg shadow-accent/30" 
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={Math.max(450, topSuppliers.length * 45)}>
            <BarChart 
              data={topSuppliers} 
              layout="vertical" 
              ...
              <Tooltip 
                ...
                content={({ active, payload }) => {
                  if (active && payload && payload.length) {
                    const data = payload[0].payload as SupplierData;
                    return (
                      <div className="bg-background/95 backdrop-blur-xl border-2 border-border rounded-xl p-4 shadow-2xl">
                        <p className="font-bold text-base mb-3 text-accent">{data.fornecedor}</p>
                        <div className="space-y-2 text-sm">
                          ...
                          <div className="flex justify-between gap-4">
                            ...
                            <span className="font-bold text-accent">{formatCurrency(data.valorTotal)}</span>
                          </div>
                          <div className="flex justify-between gap-4">
                          ...
                          <div className="flex justify-between gap-4">
                            ...
                            <span className="font-bold text-primary">{formatPercent(data.participacao)}</span>
                          </div>
                        </div>
                      </div>
                    );
        ...
        <CardHeader>
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <CardDescription className="text-base mt-2">
                Lista completa dos fornecedores com todas as métricas e indicadores
              </CardDescription>
            </div>
          </div>
        </CardHeader>
          ...
          <Table>
            ...
            <TableBody>
              {topSuppliers.map((supplier, index) => (
                <TableRow key={supplier.codPart} className="group">
                  <TableCell className="font-bold text-muted-foreground">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-accent/10 to-primary/10 flex items-center justify-center font-bold text-sm group-hover:from-accent/20 group-hover:to-primary/20 transition-colors">
                      {index + 1}
                    </div>
                  </TableCell>
                  <TableCell className="font-semibold max-w-xs">
                    <div className="flex items-center gap-2">
                      <div className={cn(
                        ...
                        supplier.acumuladoPareto <= 0.8 
                          ? "bg-gradient-to-b from-accent to-primary" 
                          : "bg-muted-foreground/30"
                      )} />
                      <span className="truncate">{supplier.fornecedor}</span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="text-center">
                    <Badge 
                      variant="outline" 
                      className="font-bold bg-gradient-to-r from-accent/5 to-primary/5 border-accent/30"
                    >
                      {supplier.uf}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell className="text-right font-semibold">{supplier.countNf}</TableCell>
                  <TableCell className="text-right font-medium text-muted-foreground">
                    {formatCurrency(supplier.ticketMedio)}
                  </TableCell>
                  <TableCell className="text-right">
                    <span className={cn(
                      ...
                      {formatPercent(supplier.participacao)}
                    </span>
                  </TableCell>
                </TableRow>
              ))}
